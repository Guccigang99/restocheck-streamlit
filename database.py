...
DB_PATH = Path("data/restocheck.db")
...
def get_restaurants(active_only=False):
    conn = get_connection()
    cur = conn.cursor()
    if active_only:
        cur.execute("SELECT id, name, address, active FROM restaurants WHERE active=1 ORDER BY name")
    else:
...
def add_crew(restaurant_id, first_name, last_name, birthdate, status, contract_hours=None,
             function="", department="", notes="", active=True):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO crew (
            restaurant_id, first_name, last_name, birthdate, status, contract_hours,
...
def update_crew(crew_id, first_name, last_name, birthdate, status, contract_hours=None,
                function="", department="", notes="", active=True):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
    ...
    """, (
        ...
        function.strip(), department.strip(), notes.strip(), 1 if active else 0, crew_id
    ))
    conn.commit()
    conn.close()
