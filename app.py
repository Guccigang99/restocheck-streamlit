...
def selected_restaurant_banner(lang):
    if st.session_state.restaurant_id:
        ...
        if resto:
            st.markdown(f"<div class='selected-resto'>🏢 {t('current_restaurant', lang)}: {resto[1]}</div>", unsafe_allow_html=True)
            return resto
    ...
    if resto:
        ...
        with add_crew_tab:
            ...
            with st.form("add_crew_form"):
                ...
                if st.form_submit_button(f"💾 {t('save', lang)}", type="primary"):
        ...
        with edit_tab:
            ...
            if not rows:
                ...
                selected_name = st.selectbox("👤 Kies crewlid", list(crew_map.keys()))
                ...
                with st.form("edit_crew_form"):
                    ...
                    with col2:
                        ...
                        new_status_label = st.selectbox(f"🏷️ {t('status', lang)}", label_values, index=label_values.index(current_label))
                    ...
                    if st.form_submit_button(f"💾 {t('save', lang)}"):
                        ...
                        st.success(f"✅ {t('saved', lang)}")
                        st.rerun()
                if st.button(f"🗑️ {t('delete', lang)} - {first_name} {last_name}"):
                    delete_crew(crew_id)
                    st.success(f"✅ {t('deleted', lang)}")
                    st.rerun()
