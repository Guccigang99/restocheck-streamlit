...
from database import init_db, get_restaurants, add_restaurant, update_restaurant, delete_restaurant, get_restaurant
...
with tab_edit:
    if restaurants:
        ...
        edit_name = st.selectbox(t("edit_restaurant", lang), list(edit_map.keys()), key="edit_restaurant_select")
        selected = get_restaurant(edit_map[edit_name])
        if selected:
            with st.form("edit_restaurant_form"):
                new_name = st.text_input(t("restaurant_name", lang), value=selected[1])
                new_address = st.text_input(t("address", lang), value=selected[2] or "")
                new_active = st.checkbox(t("active", lang), value=bool(selected[3]))
                submitted = st.form_submit_button(t("save", lang))
                if submitted:
...
with tab_delete:
    if restaurants:
        ...
        delete_name = st.selectbox(t("delete_restaurant", lang), list(delete_map.keys()), key="delete_restaurant_select")
...
st.write("Ga in de zijbalk of via het menu naar **pages/1_Crew.py** zodra een restaurant geselecteerd is.")
